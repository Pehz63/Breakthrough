...
| 56 | The cause of the non-reproducing-opponent defect (see `todo.md`) is state our side's SEARCH writes that the opponent's search then reads, so removing our search changes the opponent's reply. A second leak of the same family as theory 54 | Open. This entry is the candidate MECHANISM only. The defect it would explain is not a theory and is not tracked here: a deterministic agent that does not reproduce is broken whatever the cause, and it is logged as a defect in `todo.md`. Competing causes not yet excluded: state that is not search-related but still differs between the two runs, and the possibility that the five affected lines have five unrelated causes rather than one. The diff test named in the results doc separates these, since a single shared mechanism should show a consistent signature across the five | Gameplay Performance & Dethroning the Champion | fell out of refuting theory 55, 2026-08-29 | [refutation-oracle-results-2-quiet-lodestone](../plans/refutation-oracle-results-2-quiet-lodestone.md) |
