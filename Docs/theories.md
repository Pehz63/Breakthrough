...
| 45 | A convergence-stopped ladder whose stop threshold is smaller than its own seed noise will stop at the first rung and report a false ceiling | **Confirmed by re-analysis, 2026-07-29** (no new games needed). `train_scaling.ps1` stopped its self-play arm at 500 games on a +16 mean gain under a 20-Elo rule, but the seed spread WITHIN a size was 94 Elo (250) and 72 Elo (500), and no size above 500 was ever tested. The stored conclusion "self-play converges at 500 games" was quoted in this session as if measured; it is a stop-rule artifact. Consequence: the TD-Leaf study rates a checkpoint LADDER instead, making game count an output | Benchmarking & Measurement | caught when the developer challenged the citation, 2026-07-29 | [tdleaf-plan-1](../plans/tdleaf-plan-1-amber-pangolin.md) |
