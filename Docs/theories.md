...
| 56 | A deterministic opponent replies differently when replaying a book than it did while the line was mined, because our side SEARCHES at unbooked plies during mining and does not during playback. A second search-state leak of the same family as theory 54 | Open, and it is a deduction rather than a measurement. All five won-but-out-of-book lines on the clean A/B report `ovr_ply = -1`, meaning no position exists where the replay stands on a position the mine recorded and the book returns a different move. So OUR moves reproduced the mined line exactly up to the point the book fell silent, and the position can only have changed because the opponent replied differently. Test named in the results doc: log the opponent's chosen move per ply in both runs for one of the five and diff them to name the ply, then bisect what state that ply reads | Gameplay Performance & Dethroning the Champion | fell out of refuting theory 55, 2026-08-29 | [refutation-oracle-results-2-quiet-lodestone](../plans/refutation-oracle-results-2-quiet-lodestone.md) |
| 57 | The refutation miner's run-to-run irreproducibility comes from the 14 wall-clock-budgeted (`time=`) agents alone | Supported, not isolated. With them excluded, two separate processes running two different binaries produce byte-identical books and identical 210-row reports. With them included, two runs of the same workload differ by two book entries at the end of stage 1, and stage 2's path dependence amplifies that into nine extra concessions. An independent replay of book21 moved exactly one row of 238, a `time=150ms` core, which also shifted its own out-of-book ply from 13 to 16. Consistent with wall-clock budgets alone, but not separated from any other property those 14 agents share | Efficiency & Speed | book21 vs book22 failing to reproduce, 2026-08-29 | [refutation-oracle-results-2-quiet-lodestone](../plans/refutation-oracle-results-2-quiet-lodestone.md) |
