...
| 51 | Within the Gumbel-Zero self-play + MCTS training regime, MLP capacity substantially improves playing strength over both linear and conv, in contrast to theory 27's finding (higher-capacity MLP LOST Elo despite lower loss) under the `tdleaf_self` supervised-regression + alpha-beta regime | **Supported for mlp-vs-conv, suggestive for mlp-vs-linear, 2026-08-22.** 92 draws (45 mlp + 45 conv + 2 REF), 4 shared rungs, pinned screening fit against the same 27-agent pool: mlp's best-of-46 checkpoint (hidden=32, rung=4000) reached **1023 +/- 20 Elo, rank #2 of 395 pool agents**, beaten only by the pool's single best `ab(deep=6)` linear agent (1039); conv's best-of-46 reached only 806. The 217-Elo mlp-vs-conv gap is well outside the project's 50-150 Elo seed-noise band and holds at the population level (46 independent draws per architecture, not one seed pair), though each individual cell is still 1 seed. Against Round 5's best PURE-LINEAR Gumbel-Zero checkpoint (816 +/- 19, same regime, same pinned pool, different fit run pinned to the same nominally-unchanged `standings.tsv`): mlp's best is +207, conv's best is -10 -- suggestive that mlp help specifically (not conv) generalizes to beating linear too, but this crosses two separately-run fits not independently re-verified as sharing identical anchors, so it is reported as suggestive, not confirmed. l2=0 dominates the top of the table again (18/20 of top-20 checkpoints), replicating theory 50's underlying study's l2 pattern rather than confirming it independently. Mechanism for the regime-dependence (why capacity helps in self-play+MCTS but hurt in supervised-regression+alpha-beta) is untested | Model & Evaluator Design | follow-up round to theory 50's study, once `train.exe gumbelzero` gained `--model-type`/`--mlp-hidden`/`--conv-channels` support, developer instruction 2026-08-19 ("Run what's currently supported. Randomly sample for mlp and convolution.") | [gumbel-mcts-arch-results-6-silver-thistle](../plans/gumbel-mcts-arch-results-6-silver-thistle.md) |
