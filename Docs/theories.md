...
| 67 | Carrying a move's unspent node budget into the same side's next move (`retain`) makes the CHOICE of node count matter less, because a core that frequently leaves a large remainder can bank it and buy depth later. If so, the Elo-vs-node-count curve should be flatter under `retain` than without it | **Mechanism built and verified 2026-09-05, Elo IN FLIGHT.** New binary `ab()` flag. The purse is `g_nodeCarry[2]` (per side), reset per game beside every `ttClear`. It converges rather than exploding, because a bigger cap raises the `rem=` gate's absolute threshold until a full iteration fits and drains the purse. Measured at `rem=70,nodes=200k`: the chip counter goes 108,032 -> 165,058 mean nodes/move with a 429,035 peak and plies at depth >= 7 rising 19.7% -> 36.8%, and the tdleaf_self linear `model=169` goes 116,712 -> 174,098 with a 501,604 peak and 16.2% -> 23.7%. Mean spend stays under the nominal cap, so total per-game compute is conserved while individual moves spend 2.1-2.5 caps. The test is a 50-cell pinned cohort, 5 cores x 5 budgets (100k/200k/300k/550k/1m) x {`rem=70`, `rem=70,retain`}, read as the slope of Elo on log2(budget) per core and condition: the hypothesis predicts a smaller slope magnitude under `retain` | Efficiency & Speed | developer proposal, 2026-09-04 | [budget-parity-results-2](../plans/budget-parity-results-2-steady-meridian.md) |
