...
| 67 | Carrying a move's unspent node budget into the same side's next move (`retain`) makes the CHOICE of node count matter less, because a core that frequently leaves a large remainder can bank it and buy depth later. If so, the Elo-vs-node-count curve should be flatter under `retain` than without it | **Mechanism built and verified 2026-09-05, Elo IN FLIGHT.** New binary `ab()` flag. The purse is `g_nodeCarry[2]` (per side), reset per game beside every `ttClear`. It converges rather than exploding, because a bigger cap raises the `rem=` gate's absolute threshold until a full iteration fits and drains the purse. Measured at `rem=70,nodes=200k`: the chip counter goes 108,032 -> 165,058 mean nodes/move with a 429,035 peak and plies at depth >= 7 rising 19.7% -> 36.8%, and the tdleaf_self linear `model=169` goes 116,712 -> 174,098 with a 501,604 peak and 16.2% -> 23.7%. Mean spend stays under the nominal cap, so total per-game compute is conserved while individual moves spend 2.1-2.5 caps. **Elo settled 2026-09-05: CONFIRMED on the two cores where budget mattered, VACUOUS on the three where it did not.** 40-cell pinned cohort, 5 cores x 4 budgets (100k/200k/300k/400k) x {`rem=70`, `rem=70,retain`}, 1,328-1,358 games per cell, one fit (`--pin ranking/q5/pin_no_d12.tsv`), read as the slope of Elo on log2(budget). Slope change under `retain`, Elo per doubling: tdleaf_self lin `model=169` +103.3 -> +36.7 (-66.6+-12.2, z=-5.45), tdleaf_self lin `model=349` +81.3 -> +24.0 (-57.3+-11.9, z=-4.83), position_elo mlp `model=113` +40.0 -> +29.5 (z=-1.01), chip counter +3.3 -> +1.7 (z=-0.15), pool_games lin `model=97` +15.2 -> +18.9 (z=+0.36). The flattening raises the low end rather than capping the top: 18 of 20 cell-wise `retain` minus plain contrasts are zero or positive, and the effect is +142+-17 and +120+-17 at 100k on the two tdleaf_self cores. On both of those, `retain` at 100k is indistinguishable from plain at 200k (-2+-18 and +2+-18), so it is worth one doubling of budget. It cannot help a core with no depth slope to begin with: the chip counter's search changes measurably under `retain` and its Elo does not, which is separate evidence that it is depth-saturated at these budgets | Efficiency & Speed | developer proposal, 2026-09-04 | [budget-parity-results-2](../plans/budget-parity-results-2-steady-meridian.md) |
