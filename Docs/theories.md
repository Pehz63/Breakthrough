...
| 71 | A deterministic agent's reply is a pure function of the position, not of the path taken to it or of whether the opponent searched. This is not obvious: the transposition table is ONE process-wide, always-replace array shared by both sides, so a searching opponent evicts this agent's own entries, and `pairGameTarget` caps a deterministic pair at 2 games on the assumption that the property holds | **CONFIRMED for node-budgeted and fixed-depth agents, FALSE for `time=` agents, 2026-09-09.** A reference game was replayed under three perturbations and compared per ply by `positionKey` hash: White never searches and replays its recorded move, White is silent only for its first 8 plies (the `.opener(rand,moves=8)` shape), and both sides search normally but on a table another game already dirtied. Swept over 5 boards x 11 configurations (chip counter, `tdleaf_self lin model=169`, `position_elo mlp model=110`, with and without `tt`, with and without `qs`, on `ab(deep=6,tt,ord,nodes=200k)@3` and on both new `deep=12` heads, including the budget-BOUND case where the node cap cuts the search mid-iteration): **0 divergences in every node-budgeted and fixed-depth cell**. The project gate agrees over a wider set: `rank.exe determinism --replicas 4` across the whole deterministic roster (140 of 426 active agents, 2 colours, 1,120 games) returns **280/280 subject-colours reproducible**, and `--only deep=12` returns 70/70. That set covers the fixed-depth agents the sweep did not, since every sweep case carried a `nodes=` or `time=` budget. The only divergences were on `ab(deep=12,tt,ord,retain,time=25ms)@3`, 1 of 5 boards under a silent White and 1 of 5 under a dirty table, and they moved between runs, which is wall-clock sensitivity rather than a state leak. That case is not a defect: `rankAgentIsDeterministic` returns false for `timeBudgetMs > 0`, so the 2-game cap never applied to it. **Mechanism, narrowed 2026-09-09 after the first version overclaimed.** (1) `setTTContext` (`src/ai_minimax.cpp:196`) salts the key with root side, evaluator and eval params, so a FOREIGN entry is a MISS rather than a false hit. That closes the cross-agent channel and nothing more: `ttProbe` (`src/transposition.cpp:31-42`) does not check `e.gen`, so a search reads entries its OWN earlier searches stored, and whether those survived depends on whether the other side's intervening search evicted them. Salting says nothing about that. (2) The ROOT move loop is unordered, `orderMoves` runs only at interior nodes, so table-driven ordering cannot reorder the candidates a tie is broken among. That BOUNDS the effect at the root, it does not remove it at interior nodes, where a node budget decides where the search runs out. The eviction channel is therefore ruled out by measurement rather than by argument: **1,836 replay-games in which one side goes silent and the other keeps searching, 0 divergences**, over 37 `tt` and 4 non-`tt` roster opponents at `ab(deep=6,tt,ord,nodes=200k)@3.classic(chip=100)@2` and over the 5 actual slot-23 opponents at `ab(deep=8,tt,ord,<budget>)@3.classic(chip=100)@2` with the budget swept 200k / 800k / 2m / 8m (a 40x span, chosen because at 2m our search wraps the 2^20-slot table twice per move and wipes it while going silent leaves it whole), and in both silent-window shapes, since the first version of the probe had the window inverted. A `.dil(prob=20)@1` control diverged in every arm. Consequence to remember: **a change that orders the root would invalidate this result** and `tests/test_determinism.cpp` would need rerunning. The instrument was validated before the reading was quoted, with two positive controls now permanent in that test: the harness replaying its own reference identically, and a `.dil(prob=20)@1` opponent that must diverge, which it does at ply 1 **The observation that raised this theory turned out not to be about determinism at all.** `rank.exe refute --slot 23 --verify-only` on the current `@3` binary reproduces the 2026-08-29 `@2` run's audit trace exactly: 3, 7, 11, 12 lines leaving the book at 120 / 160 / 200 / 210 audited, 202 won, verify 202-8-0 over 3505 book entries. A result that reproduces byte for byte across eleven days and six engine commits is not a reproducibility failure. The 12 lines are a systematic difference between the mining condition (our side searches at unbooked plies) and the audit condition (the book serves instantly), and `rank.exe refute` now carries `off_ply` and `off_by` columns to report which side's move leaves the mined path, since `ovr_ply = -1` does not establish it, and a fresh mine (slot 26) says **5 of 5 won book-leavers are `off_by = 2`**: our move matched the mine at every prior ply and the OPPONENT replied differently, with `off_ply` equal to `oob_first` on all five. That leaves a tension rather than a closed case, since the 1,836-game probe removes our side's search the same way and the opponent's reply does not move. Both measurements stand, so the probe does not reproduce the harness's condition, and the next step is a per-ply dump of the mining game beside the audit game for one target | Search & Evaluation Engineering | `todo.md`'s indicated-but-unconfirmed defect, raised by 5 slot-23 book lines reporting `ovr_ply = -1` | [tests/test_determinism.cpp](../tests/test_determinism.cpp) |
| 72 | TD-Leaf's PV walk recovers a depth-d principal variation by probing the transposition table, so the gradient is taken at the leaf the search actually returned rather than at the root's successor | **REFUTED for the code as it stood, FIXED 2026-09-09.** `walkPV` (`src/ml_tdleaf.cpp`) probed the bare `positionKey(...).hash`, but since `06e4738` (2026-08-27) every entry `ttStore` writes is keyed on that hash XOR `s_ttCtx`, the searcher context mixing root side, evaluator, the full `evalParams` array and `g_useQuiescence`. Every probe therefore missed, the walk returned 0 steps at every position, and the reported diagnostic said so plainly: **mean PV depth 1.0 of 12, truncated 10724/10724 (100%)**. TD-Leaf was running as TD(lambda) on the static value of the position one ply after the root, with no error, no crash, and weights still moving. Found by Pass 1 sanity of the budget-parity Part 2 campaign, which is the pass that exists for exactly this. Fix: expose `ttSearchContext()` and XOR it into the probe. Same run, same seed, same budget afterwards: **mean PV depth 5.62 of 12**, and the self-play record shifted 122W/66B to 95W/88B. **Blast radius is zero on the roster**: all 16 `tdleaf_self` cores in `ranking/cores.txt` were trained 2026-07-30, four weeks before the salt landed, so no rated checkpoint saw the defect. It would have hit the first Part 2 retrain. Guarded by `g_tdLastMeanPV` plus an assertion in `tests/test_ml.cpp`, validated to fail at exactly `1.0 > 1.0` with the fix reverted, and by a contract test in `tests/test_ai_integration.cpp` that a salted probe hits where a bare one misses | ML Training Systems | Pass 1 sanity, budget-parity plan Part 2 | [tests/test_ml.cpp](../tests/test_ml.cpp) |
| 73 | At equal games per agent, a sparse, well-connected random opponent graph rates each agent about as precisely as a round robin, so the games an agent needs depend on the precision wanted, not on pool size [Shah2016] | **SUPPORTED on Round 4's rung-16 games, 2026-09-13, at 128 opponents or fewer.** Subsampling the 198 category agents' pairs and refitting pinned at prior 0.01: at equal games per agent, more opponents with fewer games each gave equal or lower disjoint-draw error than fewer opponents with more games (about 285 games per agent: 16 x 16 -> 40.9 Elo, 32 x 8 -> 35.8, 64 x 4 -> 28.3, 128 x 2 -> 27.3). 128 opponents at 8 games (1,096 per agent) had 17.2 disjoint error and 15.5 RMS from the full fit, against 18.4 RMS for every opponent at 2 games (904 per agent). The best design at each budget is 1.07x to 1.5x above the Bradley-Terry SE. No design below the full data reproduced the 6 cell champions, which sit 2 to 14 Elo apart. Stratifying opponents by Elo band made no consistent difference. With one panel shared by every agent (the replication study's design, prior 0.1), opener agents' within-cell contrast error depended on games per agent and not consistently on how they were split over a panel of 16 or more (about 480 games per agent: 32 x 16 -> 17.9, 64 x 8 -> 22.1, 112 x 4 -> 23.0), while openless agents still needed more panel members. One draw pair per design, so neighbouring cells are within noise | Elo / Tournaments | `plans/sparse-schedule-plan-1-cedar-lynx.md` | [plans/sparse-schedule-results-1-cedar-lynx.md](../plans/sparse-schedule-results-1-cedar-lynx.md) |
| 74 | The fit's per-pair prior (0.5 virtual games at 50% per played pair) makes a rating depend on games per pair, not only on results | **CONFIRMED 2026-09-13.** Refitting Round 4's cohort pairs with only 2 games each moved the dil20 cells' mean up 60 to 70 Elo and the openless cells' down about 10 against the 16-games-per-pair fit, the same whether the first, last or a random 2 games were kept, and the shift halved with each doubling of games per pair (rung 8's 4 per pair +30, rung 16's 8 per pair +11 on dil20). At 16 games per pair, prior 0.5 -> 0.01 moved cell means -15.5 to +8.3 and the openless node leader +39 Elo, with no champion or core rank change. `rank.exe rate --prior X` now sets it per run | Elo / Tournaments | the sparse-schedule test's first run | [plans/sparse-schedule-results-1-cedar-lynx.md](../plans/sparse-schedule-results-1-cedar-lynx.md) |
...
in a theory's Notes; resolve the key to a full citation here. Full notes on
each source live in [works-cited.md](works-cited.md).

- `[Shah2016]` Shah, N. B., Balakrishnan, S., Bradley, J., Parekh, A.,
  Ramchandran, K. and Wainwright, M. J. (2016). "Estimation from Pairwise
  Comparisons: Sharp Minimax Bounds with Topology Dependence." JMLR 17.
  arXiv 1505.01462.
