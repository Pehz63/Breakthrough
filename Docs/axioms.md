...
| E6 | Wall/column structure terms, as implemented, add little or nothing. | N/A | Medium | The hill climber's best mixes and the reigning champion converged on `w0,l0` (zero wall, zero column). Confound: only the orthogonal/same-file forms were tested. The diagonal-defense variant (the piece that actually recaptures after a capture, per Lemma C's geometry) is untested, see the Heuristic Evaluator Feature Ideas in [todo.md](../todo.md). |
