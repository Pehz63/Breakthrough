...
| E3 | Material advantage is strongly predictive of winning. | N/A | High | Chip-weighted evaluators have dominated every pool. The reigning champion's weights are `classic(chip=100)`: material and turn only. "A 3-chip lead is usually decisive by the midgame" ([terminology.md](terminology.md)). |
...
| E7 | Forced-random opening plies objectively damage a position. | N/A | High | A positionally-aware judge scored the champion worse off on ~64% of its forced-random opener plies (mean delta +54, 60/60 games hurt), and the champion drops ~217 Elo when rostered with `.opener(rand,moves=6)@1` (1140 -> 923). Theory 6 in [theories.md](theories.md), tested in [opener-bias-results-1](../plans/opener-bias-results-1-synchronous-stearns.md). |
