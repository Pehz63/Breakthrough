...
| `cvisit=` / `cscale=` / `m=` (root breadth / c_visit / c_scale, search-time not training-time) | exposed 2026-08-17 as per-agent `gaz(sims=N,cvisit=C,cscale=S,m=M)@1` roster knobs (default 50/1.0/16, the paper's own values). Swept same day (theory 49) on R17/rung4000 (slot746) at sims=200: `m` in {2,4,8,16,32} (free), then `cvisit` in {10,50,200,500,1000,2000} x `cscale` in {0.5,1.0,3.0,5.0,10.0,15.0} (m held at 16), then `m` re-isolated at the found corner | `m<=2` costs 300+ Elo regardless of cvisit/cscale (not confounded, checked across a wide cvisit/cscale range at m=2); with m=16 fixed, cvisit and cscale both rise past the paper defaults, peaking near cvisit=500/cscale=5.0 (903 vs REF's 800-804); m=16/m=8 tied as best at that corner, m=32 no better, m=4 worse | Serving-time-only screening: yes, cvisit~500/cscale~5.0/m in [8,16] beats the paper defaults on this checkpoint. Not yet checked on a different checkpoint or sims value, so generality is open |
