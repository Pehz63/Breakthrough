...
(avg plies, end-piece margin, W-L by color, nodes/move). A wide companion,
`plans/gumbel-mcts-joint-sweep-agents-5-violet-harbor.wide.tsv`, collapses
each draw's 4 rungs into one row (101 rows) with rung-suffixed result
columns (`elo_rung100`/`elo_rung400`/`elo_rung1500`/`elo_rung4000`, and
likewise for pm/cpu/playstyle/slot/model hash), so a draw's whole ladder
reads across one line instead of four. Both generated by
...
**Peak-Elo predictor**: `plans/gumbel-mcts-peak-elo-predictor-5-violet-harbor.md`
fits a decision tree, a random forest, and a linear baseline (plus raw
per-value bucket means) predicting each draw's peak Elo from its 9
jointly-swept config axes, generated from the per-checkpoint TSV above by
`analysis/predict_peak_elo.py` (see `analysis/README.md`). Headline finding:
`l2` dominates every importance measure by roughly an order of magnitude
(l2=0 mean peak Elo 721 vs. 614 at l2=0.0003 vs. 568 at l2=0.001, over 32/27/42
draws respectively), consistent with the l2 observation above; the paired
replay-buffer axis (`replaycap`/`replaywarm`, perfectly coupled in this sweep)
is a distant second (500/2000/8000 -> mean 608/615/671); `cscale` is a weaker
third signal (mean 632/644/630/611 at cscale=40/70/100/130, and it is the
tie-breaking split inside the tree's l2>0 branch); `sims`, `lr`, `batch`,
`open`, `cvisit`, and `m` show no effect that clears the noise floor at this
sample size. Full caveats (n=101, single seed, predicts peak not final-rung
Elo, correlational, linear/from-scratch scope only) are in that document.
