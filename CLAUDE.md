...
- **Correct older documents in place with a `[HINDSIGHT <date>]` tag.** When you learn that something already written here is wrong, overstated, or no longer applies, do not leave the old text standing and record the correction only in the new document. Go back and tag the old claim. Format: `**[HINDSIGHT 2026-07-29]**` inline, or a `>` blockquote for a longer correction, placed immediately at the claim, never collected in a section at the bottom that a reader may never reach. Say (a) what the old text actually established versus what it was read as establishing, and (b) what does and does not transfer to the new situation, since "this was too weak to conclude" and "this does not apply here at all" are different corrections and a reader needs both. Keep the original text visible rather than deleting it: the wrong claim is the reason the tag exists, and later readers need to recognise it if they meet it quoted elsewhere. `grep -rn "HINDSIGHT"` is the audit.
  - **Tag every place the claim can be QUOTED FROM, not just where it was analysed.** This is the load-bearing half of the rule. Concrete instance: `plans/training-sweep-results-1-luminous-snail.md` item 3 correctly recorded in 2026-07-24 that the self-play convergence stop "triggered on noise, not on convergence" and warned "do not trust 'self-play plateaus at 500'". On 2026-07-29 that exact claim was nonetheless asserted as measured fact while planning the TD-Leaf study, because the places a reader actually reaches for it -- `tools/CLAUDE.md`'s script row, the script's own header comment, `todo.md`'s summary paragraph -- carried the headline with no caveat. A correction that lives only in a results doc does not travel. (Developer instruction, 2026-07-29.)
