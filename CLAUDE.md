...
| `analysis/`, `train_py/`, `requirements.txt` | Optional Python layer: DuckDB queries, model export contract, interpretable best/worst-value prediction over a cohort sweep for any per-checkpoint target -- Elo, speed (`cpu_ms_move`), or efficiency (`analysis/predict_peak_elo.py`) |
