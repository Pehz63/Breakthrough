...
- **Explain in chat first, then ask.** Non-trivial questions are two separate operations: first a normal chat message with the full explanation, ending the turn with no question in it (not even a prose "Question: ..."), then the AskUserQuestion multiple-choice tool in the following turn. Never combine them in one turn (text sent alongside the question widget is invisible in the developer's client), replace the widget with a prose question, or stuff the explanation into the widget's option descriptions.
