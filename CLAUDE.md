...
- **Champion declaration and ranking-claim hygiene.** `ranking/CHAMPION.md` is the single source of truth for who each category champion (the standing dethrone target for its category) is. Since 2026-07-28 the throne is split into parallel categories rather than one single champion. As of 2026-08-24 that is 6 categories: 3 divisions (openless / `.opener(rand,moves=8)@1` / a 20% full-random dilution division, `dil(prob=20)@1`) crossed with 2 compute-normalization tracks (`nodes=200k` / `time=150ms`), defined in `CHAMPION.md`'s summary table; the earlier 4-book/8-book/4-random categories are demoted to a "Deferred categories" appendix (ladder/study data, not deleted) pending a future book-mining redesign; docs reference that file instead of embedding champion Elo numbers, and a doc that must quote a number tags it with the fit date. Rules for any claim about the ranking: (1) each category champion is the highest pooled-Elo target-class agent within its category (an agent's `.opener(...)` ID segment decides category membership, see `CHAMPION.md`) in the one full-roster anchored refit, with reference-class agents (currently the d8/nb2m oracle) excluded and named in CHAMPION.md; (2) top-of-table claims require every contender pair at >= 32 games -- boost via `ranking/roster_top.txt` (keep its list current) and refit; never conclude from 8-games/pair fills, which have twice inverted at 32 games/pair; (3) never compare absolute Elo across fits (the Bradley-Terry prior compresses the scale as the pool grows, see `Docs/benchmarking.md`) -- compare order and error bands within one fit; (4) when the top may have changed (a new agent rates near the top, or a cohort joins the pool), re-certify and update `ranking/CHAMPION.md` and `todo.md`'s Agent Track goal paragraph in the same session; (5) **read standings from `ranking/standings.tsv`, not `ranking/ratings.tsv`.** Both are gitignored as of 2026-08-01, so in a fresh clone they do not exist until you run `rank.exe rate` (about 20 seconds, deterministic) -- regenerate rather than reaching for a stale copy, and note that a clone without the untracked `ranking/matches.retired_*.jsonl` parts rates on rostered games only. `ratings.tsv` is the full historical fit and includes retired agents (`active` = `gone`: superseded `@N` code versions frozen at whatever game count they had when they left the roster). Quoting a retired row as current strength is wrong, and it has already produced a wrong conclusion (a `gone` classic row at 1081 vs its live identity at 990 -- a 91-Elo phantom gap). `standings.tsv` is written by the same fit with active agents only, grouped by search head; (6) **an agent is search + evaluator, so compare only within ONE search head.** `ab(d6,tt,ord,nb200k)` and `ab(d6,ord,nb200k)` are different agents; a table mixing heads is not an evaluator comparison. State the head explicitly in any standings table, and give full canonical IDs rather than abbreviations. This rule governs what a written comparison may conclude about an evaluator, not who is eligible to hold a category title -- title eligibility is governed separately by `ranking/CHAMPION.md`'s reference-class rule, which excludes only compute-scaling on an established technique, not a different search head as such; (7) **count DISTINCT games, not stored rows.** `rand()` is consumed only by dilution and random-move agents, so a pair with neither has an inert seed and replays one game per colour however many rows the store holds. Before quoting any record or trusting any error bar, count distinct trajectories (colour + plies + result + both node totals) and report both numbers, as in "25-7 nominal, 8-4 on 12 distinct games". Median across the store is 0.438 distinct games per row, so printed `pm` is understated by roughly 1.5x and far more for deterministic pairs. Full explanation and the repair options: `Docs/benchmarking.md`, defect 3.
...
- **Match scheduling ladders by default, and the stopping rule is written before the run.** `--games N` is a TARGET, not an increment: `rankSchedule` counts stored games per pair and issues only the deficit, so rungs 2, 4, 8, ... N play exactly the same games as one pass at N (asserted in `tests/test_ranking.cpp`, "a 2,4,8 ladder plays exactly the games of one pass at 8"). `rank.exe play` and `tools/run_rank.ps1` therefore ladder by default, printing each rung's rate and the exact games remaining. Do not pass `--no-ladder` / `-NoLadder` without a reason. Two rules govern what may be done with an early rung: (1) **a mid-ladder fit is a rung, not a result.** Quote it only with the rung number and per-cell game counts inline, and never as the study's answer, since a cell below a few hundred games is the `PINNED AT LOW GAME COUNT` hazard (`Docs/corrections.md`) and has moved by up to 59 Elo on the way to settling. (2) **The ladder never stops early on its own, and neither should you improvise a stop.** Fitting after every rung and stopping when the answer looks good is optional stopping, which inflates false positives in a way error bars do not show. Write the contrast and its target SE into the plan BEFORE launching, then stop against that. Full procedure and the cases where laddering does not apply: `plans/ranking-run-scheduling-plan-1-tidal-lantern.md`.
...
- **Writing style:** Avoid semicolons and em dashes. Use a comma or period instead, restructuring the sentence if needed. Avoid special Unicode characters like arrows or comparison signs. Use standard keyboard equivalents instead, such as `->` for a right-pointing arrow and `>=` for a greater-than-or-equal sign.
  ...
  - **Lead with levels, not derived statistics, and give more tables rather than fewer.** The first table in any answer is the raw measured quantity at each condition (the Elo grid, the us/node grid, the win rates), because that is what answers "which is better, and by how much". Slopes, regression coefficients, ratios and z-scores are summaries of that grid and belong AFTER it, never instead of it. Two failure modes this prevents, both observed on 2026-09-05 in the same reply: a slope table can carry a sign convention where the good result prints as a large negative number, and a single fitted line can hide that the underlying curve is concave and the whole effect lives in one step. When unsure which cut of the data is the useful one, print several tables. Compute is cheap and so is a table, and a reader who has the levels can always derive the summary, while a reader who has only the summary cannot recover the levels. (Developer instruction, 2026-09-05: "either give more helpful tables or if you're not sure which table would be helpful, just give more tables.")
  - **Name the baseline a comparison is against, and check that it is the right one.** A delta is meaningless without saying what it is a delta FROM, and "the obvious baseline" is often not the engine's default. Concrete instance: the `retain` study measured `rem=70,retain` against `rem=70` across 40 cells and reported "worth one doubling of the node budget" without ever playing `rem=0`, which is the actual default, so the headline was relative to a setting the developer had not chosen. Before running a grid, write down what the control condition is and why it, rather than the default, is the right zero.
...
| `analysis/`, `train_py/`, `requirements.txt` | Optional Python layer: DuckDB queries, model export contract, interpretable best/worst-value prediction over a cohort sweep for any per-checkpoint target -- Elo, speed (`cpu_ms_move`), or efficiency (`analysis/predict_peak_elo.py`) -- and standard charts (Elo-vs-axis, rung-ladder learning curves, seed-spread) rendered from the same cohort export (`analysis/plot_cohort_results.py`), and inverse-variance pooling of a per-cell Elo contrast across cores with Q and I^2 heterogeneity reported (`analysis/pool_retain_effect.py`, the backing for theory 70), and the replication study's Stage 1 analysis: Dunnett contrasts against the baseline, bootstrap intervals over seeds, compute multiplier, learning-curve area and speed-vs-ceiling fits, with a synthetic self-test (`analysis/replication_stage1.py`), and sparse opponent-graph schedules tested by subsampling a round robin's stored games and refitting each subsample (`analysis/sparse_schedule_sim.py`, with `analysis/sparse_prior_diag.py` separating the fit's per-pair prior from game timing) |
