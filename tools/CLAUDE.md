...
| `replication_pass2.ps1` | Replication study Stage 1, Pass 2's rated steps. `-Step curve` (every arm, 1 seed, game ladder `-CurveRungs`, at the middle of its locked learning-rate range: curve shape and CPU seconds per game), `-Step noise` (`-NoiseArms B0,<arm>`, `-NoiseSeeds` seeds at `-NoiseCpu` matched CPU seconds, converted to each arm's game count off its curve run: sigma_seed and sigma_meas), `-Step tune` (the same `-Draws` random draws for every arm over its locked range, twice as many joint draws for arms with an arm-specific hyperparameter: lambda for B0/A1, d_min in {1,2,4,8} for A3, epsilon in [0.01, 0.3] for A7, ordinal start e for A8, with A5..A8 at `-BaseLambda`, B0's tuned lambda). Draws are compute-matched per draw, not per arm: A3's cost per game depends on d_min, so each A3 draw's game count comes from the priced curve ladder of its own d_min, trained as the arm names `A3d2`, `A3d4`, `A3d8` (A3's recipe at that d_min, under its own run keys). `-WidenArms B0,A3 -WidenDraws 8 -WidenUp 0.5` adds 8 more draws per named arm in the half decade above its locked range, numbered from 101, for an arm whose best draw sat at the top of the range. Every rated checkpoint is published to a slot (ledger `models/sweep/replication_stage1_pass2.csv`, from 1876), wears `-Opener` (default `.opener(rand,moves=8)@1`, so panel pairs are never deterministic) and plays ONLY the `-Panel` roster: one `rank.exe play --paired-openings --common-openings` process per agent, into its own part under `ranking/matches_rep1/`, listed in `ranking/matches_rep1.index.txt`. One `rate --pin <-PanelPin>` over the study store then rates everything (`ranking/standings_rep1_pinned.tsv`), and the report prints levels tables, exports `analysis/replication_stage1.py` runs and runs `verify-store`. Phases `train`, `publish`, `play`, `rate`, `report`, `all`, all resumable. Every panel id must be in `-PanelPin`. `-CurveTag <suffix>` runs a second curve ladder under its own run keys (no collision with the published one, its own report column), and `-TrainExe` names the trainer binary. The `cpu=` stamps depend on machine load (`src/CLAUDE.md`, `train_budget.cpp`), so compute is matched in PRICED seconds: `-Step cost` resumes each arm's published curve checkpoints at `-CostFrom` rungs for a few games (`-CostReps` repeats), holding exactly `-CostSlots` trainings running with filler runs, and refuses to start while any other trainer runs (`-CostAllowBusy` is for stand-in tests only). It fits CPU microseconds per search node, one price shared by every arm without a TreeStrap walk (`-CostPricing shared`, default) or one per arm (`perarm`), and for A3 that per-node price plus a price per TreeStrap probe move (`treemoves=`), into `<Work>/cost_prices.tsv` with `cost_bench.tsv` beside it. `-Step cost -Phase report` refits from the stored `cost_bench.tsv` without rerunning jobs. `-NoiseCpu`/`-TuneCpu` convert through those prices, and the curve report adds priced-seconds and Elo-at-matched-priced-seconds tables. A treestrap checkpoint trained before the `tree=` stamp borrows counts from another ladder's checkpoint with identical weights at the same game count. |
