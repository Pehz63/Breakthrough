...
## Gumbel MCTS: search + joint policy/value model + self-play trainer
...
**Self-play trainer (`gumbelzero` regime, Pass 1 sanity only, `src/ml_gumbelzero.h`/`.cpp`).**
One evolving `joint` model plays both sides; no game-outcome label is used at
all. At every ply, `gumbelSearch`'s own `GumbelRootInfo` supplies both
training targets directly, so nothing waits for the game to finish:

- **Value target**: `(GumbelRootInfo::searchValue + 1) / 2`, in `[0,1]` -- the
  root's mean backed-up value over every simulation that ply (the search's
  OWN improved estimate), not `rootValue`'s single pre-search network read
  and not the eventual game outcome. Trained by the same sigmoid
  cross-entropy every other value regime here already uses
  (`Model::trainStep`).
- **Policy target**: `gumbelImprovedPolicy(info)` --
  `softmax(logits + sigma(completedQ))` over every legal root move, using the
  search's final stats. Trained by softmax cross-entropy against the policy
  head's own current group softmax (`gumbelPolicyGradients`, a closed-form
  gradient unit-tested the same way `ml_tdleaf.h`'s `tdLeafGradients` is).

Updates are strictly online (one step per new ply) but drawn from a
fixed-capacity replay buffer (`GumbelZeroReplayBuffer`) that mixes recent
...
**What Pass 1 verified** (`tests/test_gumbelzero.cpp`): closed-form checks on
`gumbelPolicyGradients` and `gumbelImprovedPolicy` against independent
reference computations; replay-buffer eviction/sampling; a knob-validation
check that `searchValue` actually diverges from `rootValue` as simulations
run; an end-to-end run confirming weights move off their zero-initialized
start and the checkpoint is loadable and playable through the real
`gaz(...)` search path; and a two-seed check confirming the training
randomness is genuinely live. A manual `train.exe gumbelzero` run (40 games,
`sims=50`) published two checkpoints (`models/sweep/slot650.txt`,
...
| conv | yes | Small residual conv tower (3x3 same-pad layers) over the v2 board's white/black occupancy planes + a dense head; side-to-move rides in at the FC stage. Value head only (the policy head's move features aren't spatial). AlphaZero-style capacity arm. |
